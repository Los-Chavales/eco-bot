...
//Pines del servo para compuerta y compactador
Servo servo1;
...
void executeMovement(String command) {
  if (command == "FORWARD") {
    ...
    stopMotors();
    activateCollector(false);
    activateCompactor(true); // Activar compactor y compuerta
...
// compactador y compuerta
void activateCompactor(bool on) {
  delay(15000);
  for (int i = 0; i <= 180; i++){
    servo1.write(i);
    servo2.write(i);
