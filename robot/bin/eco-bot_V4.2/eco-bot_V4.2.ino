...
const unsigned long COMPACTOR_FORWARD_DURATION = 2000; // 2s motor compactador hacia adelante
const unsigned long COMPACTOR_WAIT_DURATION = 3000;    // 3s de espera entre movimientos del compactador
const unsigned long COMPACTOR_BACKWARD_DURATION = 2000; // 2s motor compactador hacia atrás
...
void checkWiFiConnection() {
  if (WiFi.status() != WL_CONNECTED) {
    ...
    if (lastWiFiStatusPrint == 0 || millis() - lastWiFiStatusPrint >= WIFI_STATUS_INTERVAL) {
